...
• 'Idoso' se 60+ */
create view mostrarr41 as
	select nome,nascimento,
    case
		when nascimento<'1995-1-1' then 'Jovem'
        when nascimento between '1995-1-1' and '1966-1-1' then 'Adulto'
        else 'Idoso'
    end as faixa_etaria
from pessoas;
select*from mostrarr41;
...
SELECT 
    AVG(altura) AS media_altura_feminina
FROM pessoas
WHERE sexo = 'F';
...
SELECT 
    nome,
    DATE_FORMAT(nascimento, '%W') AS dia_semana_nascimento
FROM pessoas;
