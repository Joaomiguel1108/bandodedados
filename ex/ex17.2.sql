...
select produto.nome, categorias.nome_categoria
from categorias
join produto on produto.idproduto = categorias.idproduto;
...
select produto.nome, vendas.quantidade
from produto
join vendas on vendas.idvenda = produto.idvenda;
...
select departamentos.departamentos, funcionarios.nomefuncionario
from departamentos
join funcionarios on funcionarios.idfuncionario = departamentos.idfuncionario;
...
select aluno.nome_aluno,disciplina.nome_disciplina,professores.nome_professor
from aluno
join disciplina on aluno.id_disciplina = disciplina.id_disciplina
join professores on disciplina.id_professor = professores.id_professor;
...
select itens_pedido.precounitario, pedidos.datapedido
from itens_pedido
...
select matricula.idestudante, curso.nomecurso
from matricula
...
SELECT filmes.titulo AS filmes, atores.nomeator AS atores
FROM filmes
JOIN filmeatores ON filmes.idfilme = filmeatores.idfilme
JOIN atores ON filmeatores.idautores = atores.idautores;
...
select subcategorias.nome_subcategoria, categoriasproduto.nomecategoria
from subcategorias
join categoriasproduto on categoriasproduto.idcategoria = subcategorias.idcategoria;
/*14. Artigos e Tags 
